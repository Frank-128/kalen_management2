...
function Main() {
  ...
  return (
    <div className="md:basis-3/4 bg-slate-100">
      <div className=" top-0 bg-slate-300  text-center w-screen sm:w-full flex  p-12 md:px-24 px-2 h-12 justify-between items-center border-slate-400 border-b-2">
        <div>
          <span className="md:hidden p-3" onClick={() => setSidebar(!sidebar)}>
            {sidebar ? <Close /> : <Menu />}
          </span>
        </div>
        <h3 className="text-blue-950 text-sm md:text-lg xl:text-2xl p-3">
          Kalen Technologies Solutions
        </h3>
        <Popup
          trigger={
           <span className=" flex  p-full items-center gap-2 p-3">
              <h1 className="font-bold hidden md:block">{loggedUser.name}</h1>
              <img
